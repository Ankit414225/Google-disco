...
- When creating flashcards, keep questions simple and answers precise
    ...
    def get_required_mcps(self, user_prompt: str) -> List[str]:
        ...
        # Concept explanation
        if any(word in prompt for word in ["explain", "what is", "how does"]):
            mcps.append("search")

        # Summarization
        if any(word in prompt for word in ["summarize", "summary", "short notes"]):
            mcps.extend(["documents", "summarizer"])

        # Flashcards / revision
        if any(word in prompt for word in ["flashcard", "revise", "revision", "memorize"]):
            mcps.extend(["documents", "flashcards"])
